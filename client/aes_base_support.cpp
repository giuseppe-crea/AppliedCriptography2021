                ...
                unsigned char *tag)
{
    ...
    if(!(ctx = EVP_CIPHER_CTX_new())){
        printf("%sERROR: failed context initialization.%s\n","\e[0;31m","\x1b[0m");
        return 0;
    }

    //Initialise the encryption operation.
    if(1 != EVP_EncryptInit_ex(ctx, EVP_aes_256_gcm(), NULL, NULL, NULL)){
        printf("%sERROR: failed encryption initialization.%s\n","\e[0;31m","\x1b[0m");
        return 0;
    }

    //Set IV length if default 12 bytes (96 bits) is not appropriate
    if(1 != EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, iv_len, NULL)){
        printf("%sERROR: failed IV length initialization.%s\n","\e[0;31m","\x1b[0m");
        return 0;
    }

    //Initialise key and IV
    if(1 != EVP_EncryptInit_ex(ctx, NULL, NULL, key, iv)){
        printf("%sERROR: failed IV and key initialization.%s\n","\e[0;31m","\x1b[0m");
        return 0;
    }
    
    //Provide any AAD data. This can be called zero or more times as required
    if(1 != EVP_EncryptUpdate(ctx, NULL, &len, aad, aad_len)){
        printf("%sERROR: failed AAD data initialization.%s\n","\e[0;31m","\x1b[0m");
        return 0;
    }
    ...
    if(1 != EVP_EncryptUpdate(ctx, ciphertext+parsed, &len, plaintext+parsed, plaintext_len - parsed)){
        printf("%sERROR: failed encryption update.%s\n","\e[0;31m","\x1b[0m");
        return 0;
    }
   ...
   //Finalise the encryption
    if(1 != EVP_EncryptFinal_ex(ctx, ciphertext + len, &len)){
        printf("%sERRROR: failed ciphertext encryption finalization.%s\n","\e[0;31m","\x1b[0m");
        return 0;
    }
    ...
    if(1 != EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, 16, tag)){
        printf("%sERROR: failed in setting TAG.%s\n","\e[0;31m","\x1b[0m");
        return 0;
    }
                ...
                unsigned char *plaintext)
{
    ...
    if(!EVP_DecryptInit_ex(ctx, EVP_aes_256_gcm(), NULL, NULL, NULL)){
        printf("%sERROR: failed decryption initialization.%s\n","\e[0;31m","\x1b[0m");
        return 0;
    }
    ...
    while(parsed <= ciphertext_len-128){
        if(!EVP_DecryptUpdate(ctx, plaintext+parsed, &len, ciphertext+parsed, 128)){
            printf("%sERROR: failed decryption.%s\n","\e[0;31m","\x1b[0m");
            return 0;
        } else{
            parsed += 128;
            plaintext_len += len;
